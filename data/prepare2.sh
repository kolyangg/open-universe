...
# ─────────────── activate the "universe" env ────────────────
if command -v conda &>/dev/null && conda env list | awk '{print $1}' | grep -q '^universe$'; then
    # conda is always able to activate the env, even if you installed it with mamba
    set +u
    source "$(conda info --base)/etc/profile.d/conda.sh"
    conda activate universe
    set -u 2>/dev/null || true
else
    echo "❌  'universe' environment not found under conda; aborting."
    exit 1
fi
