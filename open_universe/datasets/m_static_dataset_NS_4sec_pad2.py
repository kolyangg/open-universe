...
class NoisyDataset(Dataset):
    def __init__(
        ...
        if self.audio_len is not None:
            ...
            print(f"Filtered files: {len(self.file_list)} files below {audio_len} sec.")
            
        self.do_fixed_len = True # fixed_len batches
    ...
    def __getitem__(self, idx):
        ...
        if self.clean_available:
            ...
            clean, sr2 = torchaudio.load(clean_path)
            assert sr == sr2
        else:                       # keep tensor interface even w/o clean speech
        ...
        #if self.split == "test":       # full‑length clips, no padding
        if self.split in {"val", "test"}:       # full‑length clips, no padding
            mask = torch.ones(noisy.shape[-1], dtype=torch.float32)
            return noisy, clean, transcript, mask
        ...
        #             [torch.ones(ori_len), torch.zeros(pad_amount)]
        #         ).float()
        #     else:
        ...
        #         valid_mask = torch.ones(self.audio_len, dtype=torch.float32)
        
        if self.do_fixed_len:
            ori_len = noisy.shape[-1]
            if ori_len < self.audio_len:
                pad_amount = self.audio_len - ori_len
                noisy = torch.nn.functional.pad(noisy, (0, pad_amount))
            ...
            else:                       # ori_len == self.audio_len (longer clips were filtered)
                valid_mask = torch.ones(self.audio_len, dtype=torch.float32)
        
        
        else: # audio_len==None  ➜  no padding
            valid_mask = torch.ones(noisy.shape[-1], dtype=torch.float32)
