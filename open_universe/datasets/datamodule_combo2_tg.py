...
import bisect ## ADD 06 MAY

# --------------------------------------------------------------------- #
#                         COLLATOR FACTORY                              #
# --------------------------------------------------------------------- #
def make_collator(do_print: bool, do_wandb: bool):
    # accepts either 4-tuple  (noisy, clean, txt, mask)
    # or     5-tuple  (..., tg) returned by the new dataset
    # def _collate(batch: List[Tuple[torch.Tensor, torch.Tensor, str, torch.Tensor]]):
    def _collate(batch):
       #  noisy, clean, txt, mask = zip(*batch)
        
        if len(batch[0]) == 5:                     # new path with TextGrid
            noisy, clean, txt, mask, tg = zip(*batch)
        ...
        if len(batch[0]) == 5:
            return noisy_b, clean_b, list(txt), mask_pad, list(tg)
        return noisy_b, clean_b, list(txt), mask_pad
...
### ADD 06 MAY ###
class RandomBucketBatchSampler(_BucketSampler):
    """
    ① pick one random “anchor” sample
    ② fill the batch with other still-unused samples whose length is
       within width_sec / width_pct of that anchor (random order)
    ③ if we run out of matches, take the closest-in-length leftovers
    ④ continue until the epoch is exhausted
    """
    def __init__(self, lengths, batch_size, sampler=None, **kw):
        super().__init__(lengths, sampler=sampler, **kw)
        self.bs   = batch_size
        self._L   = lengths                       # direct length lookup
        # one pass pre-sort for fast range queries via bisect
        pairs              = sorted((lengths[i], i) for i in self.ids)
        self._sorted_len   = [p[0] for p in pairs]
        self._sorted_ids   = [p[1] for p in pairs]

    # ------------------- helpers ------------------- #
    def _range_ids(self, lo, hi, remaining):
        """ids whose length ∈ [lo, hi] *and* still not used"""
        l = bisect.bisect_left (self._sorted_len, lo)
        r = bisect.bisect_right(self._sorted_len, hi)
        return [ self._sorted_ids[i]
                 for i in range(l, r)
                 if self._sorted_ids[i] in remaining ]

    # ------------------ iterator ------------------- #
    def __iter__(self):
        remaining = set(self.ids)
        wf, wp = self.width_frames, self.width_pct
        while remaining:
            anchor   = random.choice(tuple(remaining))
            rem_batch = self.bs - 1
            L        = self._L[anchor]
            lo, hi   = ((L-wf, L+wf) if wf is not None
                        else (L - wp*L, L + wp*L))

            batch    = [anchor]
            remaining.remove(anchor)

            # (a) random mates that satisfy the width constraint
            cands    = self._range_ids(lo, hi, remaining)
            if cands:
                k       = min(rem_batch, len(cands))
                picks   = random.sample(cands, k)
                batch.extend(picks)
                remaining.difference_update(picks)

            # (b) still short? —> nearest in length, regardless of width
            if len(batch) < self.bs and remaining:
                rest  = sorted(remaining, key=lambda i: abs(self._L[i]-L))
                fill  = rest[: self.bs - len(batch)]
                batch.extend(fill)
...
class DataModule(pl.LightningDataModule):
    ...
    def _train_loader(self):
        ...
        ### ADD 06 MAY ###
        if self.mode == "bucket_bs_random":
            sampler = RandomBucketBatchSampler(
                ds.lengths,
                batch_size=h["batch_size"],
                fs=ds.fs,
                width_sec=h.get("width_sec"),
