...
class NoisyDataset(Dataset):
    ...
    def __init__(
        ...
        text_path: Optional[str] = None,   # NEW parameter for transcripts
        part_used: Optional[float] = 1.0,  # NEW: parameter to use only a portion of dataset
    ):
        ...
        if part_used < 1.0:
            # Calculate number of files to keep
            n_files = len(self.file_list)
            n_keep = max(1, int(n_files * part_used))
            
            # Use only the first n_keep files
        ...
        # NEW: store the text_path if provided
        if text_path is not None:
            self.text_path = Path(to_absolute_path(text_path))
        else:
            self.text_path = None
    ...
    def __getitem__(self, idx):
        ...
        # NEW: Load transcript from the text_path folder
        if self.text_path is not None:
            ...
            transcript = ""  # or you could simply use the key
