...
class DataModule(pl.LightningDataModule):
    ...
    # ------------------------------------------------------------
    #              >>>  NEW: unique seed per worker  <<<
    # ------------------------------------------------------------
    @staticmethod
    def _seed_worker(worker_id: int):
        """
        PyTorch sets `torch.initial_seed()` differently for every
        (epoch, worker).  We forward that seed to Python’s `random`,
        NumPy **and** the dataset-local RNG stored as `dataset.rng`.
        """
    ...
    def _get_dataloader(self, split):
        return torch.utils.data.DataLoader(
            ...
            **self.cfg[split].dl_opts,
            worker_init_fn=self._seed_worker,   # NEW
        )
