...
class CrossAttentionBlock(torch.nn.Module):
    def __init__(self, hidden_dim, num_heads=4, temperature: float = 0.6): # 1.0 default (no temp)
        ...
        # --- UPD 12 MAY
        # keep the original value, but expose a mutable copy
        self.base_temperature = float(temperature)          # frozen reference
        self.temperature      = self.base_temperature       # live value
    ...
    ## ADD 12 MAY - TEMP SETTER ##
    # ──────────────────────────────────────────────────────────────
    # external hook – UniverseGAN can call this during training
    # ──────────────────────────────────────────────────────────────
    def set_temperature(self, t: float):
        """Safely update softmax temperature at runtime"""
...
class FiLM(torch.nn.Module):
    ...
    def forward(self, x, cond):
        ...
        # residual FiLM: preserve original information
        result = x + self.scale * (gamma * x + beta) # UPD 12 MAY
...
class TextConditioner(torch.nn.Module):
    ...
    def __init__(self, text_encoder_config, film_global_dim, cross_attention_dim, total_channels,
        num_heads: int, attention_temperature: float, text_encoder=None, *, blend_warmup_steps: int = 70000):
        ...
        # reuse the shared PL-BERT instance – do **not** re-instantiate
        if text_encoder is not None:
            self.text_encoder = text_encoder
        else:
            self.text_encoder = instantiate(text_encoder_config, _recursive_=False)

        # small dropout on token embeddings ≈ attention smoothing
        self.seq_dropout = torch.nn.Dropout(p=0.05) # ADD 12 MAY
        ...
        # --- new -------------------------------------------------------------
        self.blend_warmup_steps = int(blend_warmup_steps)
    ...
    def forward(self, x_mel, text, q_pad_mask=None): ### 01 MAY: ADD MASK
        ...
        ## UPD 12 MAY
        seq_emb = self.seq_dropout(seq_emb.masked_fill(text_key_mask.unsqueeze(-1), 0.0))
        ...
        ## UPD 12 MAY ##
        # linear ramp: no text-domination in first ~70 k steps
        # step_ratio = (float(getattr(self, "global_step", 0)) / 70_000.0) if self.training else 1.0
        
        
        # linear ramp: no text-domination in first ~70 k steps
        parent_step = getattr(self, "global_step", 0)        # set by UniverseGAN
        warmup_len  = max(1, self.blend_warmup_steps)        # avoid /0
        step_ratio  = (parent_step / float(warmup_len)) if self.training else 1.0
        
        
        print(f"[DEBUG] step_ratio={step_ratio:.4f}", f"global_step={getattr(self, 'global_step', 0)}")
        ramp = torch.clamp(x_mel_orig.new_tensor(step_ratio), 0., 1.)
        blend_factor = ramp * torch.sigmoid(self.text_impact_factor)
        print(f"[DEBUG] blend_factor={blend_factor.item():.4f}")
        x_mel = (1.0 - blend_factor) * x_mel_orig + blend_factor * x_mel_conditioned
...
class ConditionerNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        text_blend_warmup_steps: int = 70000, # 12 MAY ADD
    ):
        super().__init__()
        ...
        if text_encoder_config is not None:
            ...
            if self.mel_mode == "full":
                ...
                blend_warmup_steps=text_blend_warmup_steps # 12 MAY ADD
            )
    ...
    def forward(self, x, x_wav=None, train=False, text=None,
        ...
        # ADD 12 MAY
        # propagate global_step to every sub-module that may need it
        if hasattr(self, "global_step"):
            for m in self.modules():
                m.global_step = self.global_step
         # ADD 12 MAY
