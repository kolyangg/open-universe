...
import matplotlib                   # NEW 03 MAY
matplotlib.use("Agg")               # head‑less, file‑only backend
import matplotlib.pyplot as plt     # NEW 03 MAY
...
class Universe(pl.LightningModule):
    ...
    def enhance(
        ...
        tg: Optional[torch.Tensor] = None,  # ← NEW MFA
    ) -> torch.Tensor:
        """
        ...
        if text is not None:
            ...
            if tg:
                result = self.condition_model(mix, x_wav=mix_wav, text=text, train=True, mask = mask, tg = tg)
            else:
    ...
    def compute_losses(
        ...
        if self.have_text:
            ...
            if tg:
                result = self.condition_model(mix_trans, x_wav=mix, text=text, train=True, mask = mask, tg = tg) ### 15 MAY - ADD TG
            else:
    ...
    def validation_step(self, batch, batch_idx, dataset_i=0):
        ...
        if self.trainer.testing or self.n_batches_est_done < self.val_kwargs.max_enh_batches:
            ...
            if self.have_text:
                ...
                if tg:
                    est = self.enhance(mix_, rng=self.rng, text=text_, mask=mask_, tg=tg)
                else:
            ...
            for name, loss in self.enh_losses.items():
                val_metric = loss(est, target_) ## target_ is the clean target
            ...
            if self.trainer.is_global_zero and self.n_tb_samples_saved < self.num_tb_samples:
                ...
                ## UPD 10 MAY - two attention maps
                # ---------- log both MEL- and LATENT-level attention maps ----------
                att_logs = {}
                tc_m = getattr(self.condition_model, "text_cond_mel", None)
                tc_l = getattr(self.condition_model, "text_cond_lat", None)

                maps = {
                    "mel": getattr(tc_m, "last_attn_map", None) if tc_m else None,
                    "lat": getattr(tc_l, "last_attn_map", None) if tc_l else None,
                }

                for lvl, amap in maps.items():
                    if amap is None:
                        continue
                    a = amap.cpu().numpy()         # [B, Q, S]
                    for k in range(num_save):
                        fig, ax = plt.subplots(figsize=(5, 4))
                        im = ax.imshow(a[k], aspect="auto", origin="lower")
                        ax.set_xlabel("Text tokens")
                        ax.set_ylabel(f"{lvl.capitalize()} frames")
                        ax.set_title(f"{lvl.upper()}-Attn {self.global_step}_{k}")
                        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
                        att_logs[f"attention/{lvl}_{sample_id}_{k}"] = (
                            wandb.Image(fig, caption=f"{lvl.upper()} Cross-Attention {sample_id}_{k}"))
                        plt.close(fig)
                # --------------------------------------------------------------------
                ## UPD 10 MAY - two attention maps
