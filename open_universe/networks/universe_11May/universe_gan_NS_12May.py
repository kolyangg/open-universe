...
class UniverseGAN(Universe):
    def __init__(
        ...
        attention_temperature_anneal = None, ### NEW 12 MAY
    ):
        """
        ...
        # --------------------------------------------------
        # keep a handle to the anneal schedule (may be None)
        # --------------------------------------------------
        self.attn_temp_anneal = attention_temperature_anneal # NEW 12 MAY
        
        # --------------------------------------------------
        # make all ctor kwargs available via self.hparams
        # (Lightning will deep-copy non-tensor objects)
        # --------------------------------------------------
        self.save_hyperparameters(logger=False) # ADD 12 MAY
    ...
    def training_step(self, batch, batch_idx):
        ...
        # propagate global_step so sub-modules can see it    
        self.condition_model.global_step = self.global_step ## ADD 12 MAY
        #### ADD 12 MAY - ANNEAL ATTN TEMP ###
        # ─── anneal attention temperature ────────────────────────────────
        anneal = getattr(self, "attn_temp_anneal", None)
        print(f"[DEBUG] Anneal: {anneal}")
        if anneal and self.global_step >= anneal.start_step:
            prog = min(
                1.0,
                (self.global_step - anneal["start_step"])
                / (anneal["end_step"] - anneal["start_step"] + 1e-9),
            )
            # start_T = self.hparams["condition_model"]["attention_temperature"]
            start_T = self.hparams.condition_model.attention_temperature
            new_T   = (1.0 - prog) * start_T + prog * anneal["final_T"]
            print(f"[DEBUG] Global step: {self.global_step}")
            print(f"[DEBUG] Annealing attention temperature: {start_T} -> {new_T}")
        
            
            for m in self.condition_model.modules():
                if hasattr(m, "set_temperature"):
                    m.set_temperature(new_T)
        #### ADD 12 MAY - ANNEAL ATTN TEMP ###
