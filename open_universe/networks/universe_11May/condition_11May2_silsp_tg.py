...
class TextConditioner(torch.nn.Module):
    ...
    def forward(self, x_mel, text, q_pad_mask=None, tg=None): ### 01 MAY: ADD MASK
        """
        ...
        if tg:
            global_emb, seq_emb, text_key_mask, coord = self.text_encoder(text, tg) # [B,S,C]  mask: [B,S]
        else:
        ...
        #### UPD 15 MAY - ADD MFA  CALC ###
    
        # ─────────────────────────────────────────────────────────────
        # 1)  scale coords (0-1) → frame indices of x_mel_attn
        # ─────────────────────────────────────────────────────────────
        # if tg and coord is not None:
        #     T_frames = x_mel_attn.shape[1]
        #     coords_idx = []
        #     for c in coord:                         # per-batch item (List[List[2]])
        #         print(c)
        #         if c:
        #             idx = (torch.tensor(c, device=x_mel_attn.device) * T_frames).round().long()
        #             coords_idx.append(idx)          # [[B,S,2]]
        #         else:
        #             coords_idx.append(torch.zeros((0, 2), dtype=torch.long,
        #                                           device=x_mel_attn.device))
        #     self.last_coords = coords_idx           # <-- stored for loss
        # else:
        ...
        if tg and coord is not None:
            T_frames = x_mel_attn.shape[1]
            coords_idx = []
            for sample_coords in coord:             # per-sample list of token spans
                if sample_coords:
                    idx_list = []
                    for ts in sample_coords:        # ts is [] or [xmin,xmax]
                        if len(ts) == 2:
                            x0 = int(round(ts[0] * T_frames))
                            x1 = int(round(ts[1] * T_frames))
                        else:
                            x0 = x1 = 0
                        idx_list.append([x0, x1])
                    coords_idx.append(torch.tensor(
                        idx_list,
                        device=x_mel_attn.device,
                       dtype=torch.long
                    ))
                else:
                    coords_idx.append(torch.zeros(
                        (0, 2),
                        device=x_mel_attn.device,
                        dtype=torch.long
                    ))
            self.last_coords = coords_idx
        else:
            self.last_coords = None    
        ...
        #     for i, ci in enumerate(self.last_coords):
        #         if len(ci) != seq_emb.size(1):
        #             print(f"[DEBUG-ALIGN] seq_len {seq_emb.size(1)} ≠ coords_len {len(ci)} (batch {i})")
        # #### UPD 15 MAY - ADD MFA LOSS CALC ###
        ...
        if tg:
            return x_mel, text_metrics, global_emb, coord
        else:
...
class ConditionerNetwork(torch.nn.Module):
    ...
    def forward(self, x, x_wav=None, train=False, text=None,
                mask: torch.Tensor | None = None, tg = None): # UPD 15 MAY - add tg  
        ...
        if use_text and self.mel_mode != "none":
            if self.mel_mode == "full":
                ...
                if tg:
                    x_mel, text_metrics_mel, global_emb, coord = self.text_cond_mel(
                    x_mel, text, mel_pad_mask, tg=tg) # UPD 15 MAY - add tg
                else:
        ...
        if use_text and self.lat_mode != "none":
            if self.lat_mode == "full":
                ...
                if tg:
                    h, text_metrics_lat, _, coord = self.text_cond_lat(h, text, q_pad_mask, tg = tg) # UPD 15 MAY - add tg
                else:
        ...
        if train:
            if use_text:
                if tg:
                    return conditions, y_hat, h, text_metrics #, coord # coord only in train mode
                else:
