...
class TextEncoder(nn.Module):
    ...
    def forward(self, input_data: list[str]):
        ...
        ## 10 MAY - SIMPLE VERSION OF TEXTENCDOER ##

        # global_emb = self.fc_global(self.global_norm(cls_emb))
        # seq_emb = self.fc_seq(self.seq_norm(outputs.last_hidden_state))
        

        # Simplified baseline – we skip the utterance-level vector and keep
        # only per-token embeddings for cross-attention.
        seq_emb    = self.fc_seq(self.seq_norm(outputs.last_hidden_state))
        ...
        print('[DEBUG] Using a an easy TextEncoder version')
        return seq_emb, key_mask  ## 10 MAY SIMPLE VER
