...
# from .universe_NS import Universe # fixed logging
# from .universe_NS_try import Universe
from .universe_NS_try3 import Universe
...
class UniverseGAN(Universe):
    ...
    def training_step(self, batch, batch_idx):
        # Get current epoch information from PyTorch Lightning
        current_epoch = getattr(self, 'current_epoch', 0)
        global_step = getattr(self, 'global_step', 0)
        
        # Instead of trying to modify the models directly, we'll store the epoch 
        # information at the UniverseGAN level and provide a way for child models
        # to access it through their parent
        self._current_training_epoch = current_epoch
        self._current_global_step = global_step
        
        # Debug print for epoch checking
        print(f"[EPOCH CHECK] universe_gan_NS3.py - Current epoch: {current_epoch}, global_step: {global_step}")
        ...
        # Store current epoch directly as a global variable in the module
        # This is a workaround since we can't modify the score and condition models directly
        import sys
        this_module = sys.modules[__name__]
        setattr(this_module, 'CURRENT_EPOCH', current_epoch)
        setattr(this_module, 'GLOBAL_STEP', global_step)
        ...
        # Sample noise and form the noisy target.
        ...
        if self.have_text:
            # Handle the updated return value with text_metrics and text embeddings
            result = self.condition_model(mix, text=text, train=True)
            if len(result) == 5:  # With text metrics and embeddings
                cond, y_est, _, text_metrics, text_embedding = result
        ...
        # Compute the score, passing text embedding if available
        score = self.score_model(x_t, sigma, cond, text_embedding=text_embedding)
        
        # Log score text metrics if available
        if hasattr(self.score_model, 'text_score_metrics') and self.score_model.text_score_metrics:
            score_metrics = self.score_model.text_score_metrics
            for k, v in score_metrics.items():
                if isinstance(v, (int, float)):
                    self.log(f"score_text/{k}", v, batch_size=mix.shape[0], **self.log_kwargs)

        # Decouple the signal estimate if required.
        if self.signal_decoupling_layer is not None:
    ...
    def configure_optimizers(self):
        # Generator (score and condition model) optimizer.
        opt_kwargs = OmegaConf.to_container(self.opt_kwargs.generator, resolve=True)

        # Exclude keywords from weight decay.
