...
class Universe(pl.LightningModule):
    ...
    def _edm_score_wrapper(self, x, sigma, cond, with_speech_est=False, audio_mask=None):
        w = self._get_edm_weights(sigma)
        w_in = utils.pad_dim_right(w["in"], x)
        w_out = utils.pad_dim_right(w["out"], x)
        w_skip = utils.pad_dim_right(w["skip"], x)
        
        # forward audio_mask if your score network uses it
        net_out = self._edm_model(
            w_in * x,
            w["noise"] * sigma,
            cond,
    ...
    def enhance(
        ...
        text_mask: Optional[torch.Tensor] = None,  # <--- For text padding mask
        audio_mask: Optional[torch.Tensor] = None,  # <--- For audio padding mask
    ) -> torch.Tensor:
        """
        ...
        def score_wrapper(x, s, cond):
            if target is None:
                return self.score_model(x, s, cond, audio_mask = audio_mask)
        ...
        if text is not None:
            ...
            result = self.condition_model(mix, x_wav=mix_wav, text=text, text_mask=text_mask, audio_mask=audio_mask, train=True)
            ...
            cond, aux_signal, aux_latent = self.condition_model(
                mix, x_wav=mix_wav, audio_mask=audio_mask, train=True)
    ...
    def forward(self, xt, sigma, cond, audio_mask):
        return self.score_model(xt, sigma, cond, audio_mask = audio_mask)
    ...
    def compute_losses(
        ...
        audio_mask = None ## NEW WITH TEXT ENCODER ###
    ):
        ...
        if self.have_text:
            ...
            result = self.condition_model(mix_trans, x_wav=mix, text=text, audio_mask = audio_mask, train=True)
            ...
            cond, y_est, h_est = self.condition_model(mix_trans, x_wav=mix, audio_mask = audio_mask, train=True)
            self.text_metrics = {}
        ...
        if train:
            if self.losses_kwargs.weights.latent > 0.0 and h_est is not None:
                mel_target = self.condition_model.input_mel.compute_mel_spec(target_aux, audio_mask = audio_mask)
            ...
            if self.losses_kwargs.weights.signal > 0.0:
                ...
                if audio_mask is not None:
                    m = audio_mask.unsqueeze(1).to(y_est.dtype)   # (B,1,T)
                    valid_count = m.sum().clamp(min=1.0)
                    diff = torch.abs(y_est - target_aux_trans)
                    l_signal = (diff * m).sum() / valid_count
                else:
                    l_signal = self.loss_signal(y_est, target_aux_trans)
            else:
                l_signal = l_score.new_zeros(1)
            
            # Apply valid_mask to losses if provided
            if audio_mask is not None:
                # Create the expanded mask for masking operations
                valid_mask_expanded = audio_mask.unsqueeze(1)  # Add channel dimension
                valid_sum = valid_mask_expanded.sum().clamp(min=1.0)
                
                # Apply mask to the score loss if it's not a scalar
                if l_score.ndim > 1:
                    l_score = (l_score * valid_mask_expanded).sum() / valid_sum
                    
                # Apply to signal loss if needed
                if l_signal.ndim > 1 and not torch.isnan(l_signal).any():
                    l_signal = (l_signal * valid_mask_expanded).sum() / valid_sum
                    
                # Apply to latent loss if needed
                if l_latent.ndim > 1 and not torch.isnan(l_latent).any():
                    # Might need to adjust dimensions based on your specific loss shape
                    l_latent = (l_latent * valid_mask_expanded).sum() / valid_sum
            ...
            # For validation, apply mask to score loss if provided
            if audio_mask is not None and l_score.ndim > 1:
                valid_mask_expanded = audio_mask.unsqueeze(1)
                valid_sum = valid_mask_expanded.sum().clamp(min=1.0)
                l_score = (l_score * valid_mask_expanded).sum() / valid_sum
    ...
    def training_step(self, batch, batch_idx):
        ...
        if len(batch) >= 3 and isinstance(batch[2], (str, list, torch.Tensor)):
            self.have_text = True
            mix, target, text, audio_mask = batch[:4]
    ...
    def validation_step(self, batch, batch_idx, dataset_i=0):
        ...
        for i in range(self.val_kwargs.n_bins):
            if self.have_text:
                ls = self.compute_losses(
                    ...
                    audio_mask=audio_mask ## NEW WITH TEXT ENCODER ###
                )
            else: # same as in original
                ls = self.compute_losses(
                    self.pad(mix)[0],
        ...
        if self.trainer.testing or self.n_batches_est_done < self.val_kwargs.max_enh_batches:
            ...
            if self.have_text:
                mix_, target_, text_, audio_mask_ = batch[:4] 
                ...
                est = self.enhance(mix_, rng=self.rng, text=text_,text_mask=None, audio_mask=audio_mask_)
            else:
                ...
                est = self.enhance(mix_, rng=self.rng, audio_mask=audio_mask_)
