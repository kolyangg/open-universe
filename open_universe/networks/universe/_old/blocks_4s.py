...
class ConvBlock(torch.nn.Module):
    ...
    def forward(
        ...
        # adjust for padding (NEW!!!!)
        if input_cond is not None:
            # --- ALIGN THE TWO TEMPORAL LENGTHS ---
            Lc = cond_out.shape[-1]
            Li = input_cond.shape[-1]
            if Li < Lc:
                # pad right side with zeros
                input_cond = F.pad(input_cond, (0, Lc - Li))
            elif Li > Lc:
                # truncate extra samples
                input_cond = input_cond[..., :Lc]
    ...
    #         # 1) Upsample or downsample h if needed (unchanged)
    #         if self.rate_change_dir == "up":
    #             if length is not None and self.rate * h.shape[-1] < length:
    #                 h = F.pad(h, (0, 1))
    #             h = self.rate_change_conv(h)
    #             if length is not None:
    #                 h = F.pad(h, (0, length - h.shape[-1]))

    #         if res is not None:
    #             if self.rate_change_dir != "down":
    #                 h = (h + res) * norm_factor
    #             else:
    #                 raise ValueError("Residual only allowed for up blocks")

    #         # 2) First convolution
    #         cond_out = self.conv1(h)

    #         # 3) *Immediately* align input_cond to cond_out’s length
    #         if input_cond is not None:
    #             Lf = cond_out.shape[-1]
    #             Li = input_cond.shape[-1]
    #             if Li < Lf:
    #                 input_cond = F.pad(input_cond, (0, Lf - Li))
    #             elif Li > Lf:
    #                 input_cond = input_cond[..., :Lf]

    #         # 4) Combine cond_out and input_cond (or project)
    #         if input_cond is not None:
    #             if self.signal_cond_proj is None:
    #                 c = (cond_out + input_cond) * norm_factor
    #             else:
    ...
    #         # 5) Noise conditioning + two more convs
    #         if noise_cond is not None:
    #             c = film(c, noise_cond)
    #         c = self.conv2(c)
    #         c = self.conv3(c)
    ...
    #         # 6) Residual output
    #         v_out = (h + c) * norm_factor

    #         # 7) If this is a downsampling block, do the final rate_change_conv
    #         if self.rate_change_dir == "down":
    #             r = h.shape[-1] % self.rate
    #             if r != 0:
    #                 v_pad = F.pad(v_out, (0, self.rate - r))
    #             else:
    #                 v_pad = v_out
    #             h = self.rate_change_conv(v_pad)
    #             return h, v_out, cond_out

    #         # 8) Otherwise it’s just an “up” or “none” block
    #         return v_out, v_out, cond_out