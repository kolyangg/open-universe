...
from .m_universe_NS_adj3_fix_map import Universe 
...
# --------------------------------------------------------
    ...
    def configure_optimizers(self):
        ...
        # --- Only create a special text param‑group if we will **use** text ---
        
        # text branch exists only when the *original* Hydra config had the
        # `condition_model.text_encoder_config` field (→ a TextConditioner was
        # built during instantiation).
        ...
        if self.schedule_kwargs is not None:
            ...
            #         # <-- tell Lightning **which** param‑group(s) this scheduler controls
            #         "param_group_indices": [len(param_groups) - 1],   # only the text group        
            #     }
            ...
            # -- NEW: optional text warm‑up scheduler ------------------------
            # Older Lightning doesn’t support `param_group_indices`, so we
            # build a LinearLR that leaves the *other* groups unchanged by
            # giving them a start‑factor of 1.0.
