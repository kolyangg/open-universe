...
class TextConditioner(torch.nn.Module):
    ...
    def forward(self, x_mel, text, q_pad_mask=None): ### 01 MAY: ADD MASK
        ...
        seq_emb, text_key_mask = self.text_encoder(text)  
        ## 10 MAY SIMPLE UPD ##
        ...
        # 2) Prepare mel queries (no FiLM in the simplified version)
        x_mel_t  = x_mel.transpose(1, 2)            # [B,T,512]
        ## 10 MAY SIMPLE UPD ##
        ...
        self.text_impact_factor.requires_grad_(False)
