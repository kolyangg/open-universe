...
class UniverseGAN(Universe):
    ...
    def _guided_attn_loss(attn, q_mask, s_mask, sigma=0.15):
        # """
        # attn   : [B,Q,S]  – already softmax‑ed & averaged heads
        # q_mask : [B,Q]    – True for *valid* mel frames      (¬padding)
        # s_mask : [B,S]    – True for *valid* text tokens
        # """
        # B, Q, S = attn.shape
        # device  = attn.device
        # # normalised positions only for valid cells
        # i = (torch.arange(Q, device=device) / q_mask.sum(1, keepdim=True)).unsqueeze(-1)
        # j = (torch.arange(S, device=device) / s_mask.sum(1, keepdim=True)).unsqueeze( 0)
        # guide = 1.0 - torch.exp(-((i - j) ** 2) / (2 * sigma ** 2))                # [B,Q,S]
        
        # # mask out padded rows / cols *before* computing mean
        # pad = (~q_mask).unsqueeze(-1) | (~s_mask).unsqueeze(1)
        # loss = (attn.masked_fill(pad, 0.0) * guide.masked_fill(pad, 0.0)).sum() \
        #     / (~pad).float().sum().clamp_min(1.0)
