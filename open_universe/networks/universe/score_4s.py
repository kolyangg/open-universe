...
from .blocks_4s import ConvBlock, PReLU_Conv, cond_weight_norm
...
class ScoreEncoder(torch.nn.Module):
    ...
    def forward(self, x, noise_cond, audio_mask=None):
        residuals, lengths, masks = [], [], []
        m = audio_mask  # (B, T)
        for ds, lin in zip(self.ds_modules, self.cond_proj):
            # 1) record pre‑downsample length & mask
            lengths.append(x.shape[-1])
            masks.append(m)

            # 2) compute the layer’s noise conditioning
            nc = lin(noise_cond)
            x, res, _ = ds(x, noise_cond=nc)

            # 3) downsample the mask by the same stride
            if m is not None and ds.rate_change_dir=="down":
                m = F.avg_pool1d(m.float().unsqueeze(1),
                                 kernel_size=ds.rate, stride=ds.rate
                                ).squeeze(1) > 0.5

            residuals.append(res)

        # final “bottleneck” mask too
        masks.append(m)
        return x, residuals[::-1], lengths[::-1], masks[::-1]
...
class ScoreDecoder(torch.nn.Module):
    ...
    def forward(self, x, noise_cond, input_cond, residuals, lengths):
        for lvl, (up, n_lin, s_lin, cond, res, length) in enumerate(
            ...
            # 1) noise‐conditioning projection
            nc = n_lin(noise_cond)

            # 2) signal‐conditioning projection (just one argument!)
            sc = s_lin(cond)

            # 3) feed into your ConvBlock up‐module
            x, *_ = up(x, noise_cond=nc, input_cond=sc, res=res, length=length)
...
class ScoreNetwork(torch.nn.Module):
    ...
    def forward(self, x, sigma, cond, audio_mask=None):
        n_samples = x.shape[-1]
        ...
        h, residuals, lengths, masks = self.encoder(x, noise_cond=g,
