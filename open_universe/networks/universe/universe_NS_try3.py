...
# wandb_logger = pl_loggers.WandbLogger(project="universe", name=exp_name, config=cfg)
...
class Universe(pl.LightningModule):
    ...
    def _edm_score_wrapper(self, x, sigma, cond, with_speech_est=False, text_embedding=None):
        ...
        # Pass text_embedding to the EDM model if supported
        if hasattr(self._edm_model, 'forward') and 'text_embedding' in self._edm_model.forward.__code__.co_varnames:
            net_out = self._edm_model(w_in * x, w["noise"] * sigma, cond, text_embedding=text_embedding)
        else:
    ...
    ### UPDATED ENHANCE METHOD FOR PROPER HANDLING OF TEXT FEATUES
    def enhance(
        self,
        mix,
        ...
        text: Optional[List[str]] = None,  # Text transcripts
    ) -> torch.Tensor:
        if epsilon is None:
        ...
        def score_wrapper(x, s, cond):
            # Extract text embedding from condition model if available
            text_embedding = None
            if hasattr(self.condition_model, 'text_global_embedding'):
                text_embedding = self.condition_model.text_global_embedding
            
            if target is None:
                # Pass text embedding to score model if available
                score = self.score_model(x, s, cond, text_embedding=text_embedding)
            else:
        ...
        # Determine whether to use text conditioning
        has_text_conditioning = hasattr(self, 'have_text') and self.have_text and text is not None
        
        # Debug mode during validation
        debug_mode = hasattr(self, 'trainer') and not self.training
        
        # Conditioning step
        if has_text_conditioning:
            if debug_mode:
                print(f"[DEBUG] Using text in enhance with {len(text)} transcripts")
                if len(text) > 0:
                    print(f"[DEBUG] First transcript: '{text[0]}'")
                
            # Check if there are any non-empty transcripts
            has_valid_text = any(t and t.strip() for t in text)
            
            if has_valid_text:
                if debug_mode:
                    print(f"[DEBUG] Found valid text content for conditioning")
                    
                # Get conditioning with text
                result = self.condition_model(mix, x_wav=mix_wav, text=text, train=True)
                
                # Handle different return formats
                if isinstance(result, tuple):
                    if len(result) >= 5:  # With text metrics and embeddings
                        cond, aux_signal, aux_latent, text_metrics, text_embedding = result
                        if debug_mode and isinstance(text_metrics, dict):
                            blend = text_metrics.get('blend_factor', 'N/A')
                            print(f"[DEBUG] Text conditioning applied with blend factor: {blend}")
                    elif len(result) >= 4:  # With text metrics, no embeddings
                        cond, aux_signal, aux_latent, text_metrics = result
                        if debug_mode and isinstance(text_metrics, dict):
                            blend = text_metrics.get('blend_factor', 'N/A')
                            print(f"[DEBUG] Text conditioning applied with blend factor: {blend}")
                    else:
                    ...
                    if debug_mode:
                        print(f"[WARNING] Unexpected return format from condition_model: {type(result)}")
                    # Fallback
                    cond, aux_signal, aux_latent = self.condition_model(mix, x_wav=mix_wav, train=True)
            else:
                # All transcripts are empty, fall back to audio-only
                if debug_mode:
                    print(f"[DEBUG] No valid text content found, using audio-only conditioning")
                    
                cond, aux_signal, aux_latent = self.condition_model(mix, x_wav=mix_wav, train=True)
        else:
            # No text conditioning available or requested
            if debug_mode:
                print(f"[DEBUG] Using audio-only conditioning (no text capability or no text provided)")
                
            cond, aux_signal, aux_latent = self.condition_model(mix, x_wav=mix_wav, train=True)

        # The rest of the enhance method (diffusion process) stays the same
        if use_aux_signal:
    ...
    def compute_losses(
        ...
        if self.have_text:
            # THIS LINE CHANGES - Store the result which now includes text_metrics and embeddings
            result = self.condition_model(mix_trans, x_wav=mix, text=text, train=True)
            if len(result) == 5:  # With text metrics and embeddings
                cond, y_est, h_est, text_metrics, text_embedding = result
                # Store metrics for later use in training_step
                self.text_metrics = text_metrics
            elif len(result) == 4:  # With text metrics but no embeddings
    ...
    def on_validation_epoch_start(self):
        ...
        self.num_tb_samples = self.val_kwargs.get("num_tb_samples", 5) # 0
