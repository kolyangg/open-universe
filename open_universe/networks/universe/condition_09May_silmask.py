...
class TextConditioner(torch.nn.Module):
    ...
    def forward(self, x_mel, text, q_pad_mask=None): ### 01 MAY: ADD MASK
        ...
        # B = seq_emb.size(0)
        B, T_mel = x_mel_attn.shape[:2] # UPD 09 MAY to do energy based masking
        ...
        # UPD 09 MAY to do energy based masking
        # ── build a “query mask” that disables SIL once energy > thresh ──────
        # crude VAD: frame energy over feature dim
        frame_energy = x_mel_t.pow(2).mean(dim=-1)        # [B, T_mel]
        vad_mask = frame_energy > 0.02                    # tweak threshold
        # keep SIL for the first frame regardless
        vad_mask[:, 0] = False

        if q_pad_mask is not None:
            q_mask = q_pad_mask | vad_mask                # OR the two masks
        else:
            q_mask = vad_mask

        x_mel_attn = x_mel_attn.masked_fill(q_mask.unsqueeze(-1), 0.)
        # UPD 09 MAY to do energy based masking
