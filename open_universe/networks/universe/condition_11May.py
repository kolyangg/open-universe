...
class TextConditioner(torch.nn.Module):
    ...
    def __init__(self, text_encoder_config, film_global_dim, cross_attention_dim, total_channels,
        ...
        ### UPD 11 MAY - REMOVE SIL TOKEN AND POS EMB HERE (ALREADY IN PL-BERT)
        # ## ADD 09 MAY ##
        # # --- learnable SIL token ------------------------------------------
...
class ConditionerNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        if text_encoder_config is not None:
            ...
            #     attention_temperature=attention_temperature,
            # )
            self.text_cond_lat = None ## UPD 11 MAY - FILM ONLY FOR LATENT SPACE
    ...
    def forward(self, x, x_wav=None, train=False, text=None,
        ...
        if use_text:
            ...
            # x_mel, text_metrics = self.text_conditioner(x_mel, text) ## 10 MAY upd fix
            ## UPD 10 MAY ##
        ...
        if use_text:
            ...
            # text_metrics["attn_map_lat"] = self.text_cond_lat.last_attn_map
            ## ## UPD 10 MAY ##
