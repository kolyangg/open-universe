...
Modified to move cross-attention after the GRU block.
"""
import math
...
###############################################################################
# Basic building blocks used by the conditioner
###############################################################################
...
class MelAdapter(torch.nn.Module):
    ...
    def compute_mel_spec(self, x):
        ...
        x = self.mel_spec(x)  # -> (B, n_mels, frames) ignoring the batch dimension carefully
        x = x.squeeze(1)
...
###############################################################################
# Optional FiLM block and CrossAttentionBlock for text conditioning
###############################################################################
...
class FiLM(torch.nn.Module):
    ...
    def forward(self, x, cond):
        ...
        # Center gamma around 1.0
        gamma_centered = 1.0 + gamma

        out = self.global_scale.clamp(0.5, 2.0) * (gamma_centered * x + beta)
        return out, {  # Return debug stats if desired
...
class CrossAttentionBlock(torch.nn.Module):
    ...
    def forward(self, x, cond, x_mask=None, cond_mask=None):
        ...
        # Temperature scaling of attention (optional)
        temp = self.attention_temperature.clamp(min=0.01, max=1.0)
        # (If you want to sharpen the distribution, you could do something like
        # attn_weights = attn_weights ** (1.0/temp), then renormalize.)
...
class ConditionerEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        x_mel:    (B, 512, T_mel)         # from MelAdapter
        global_emb, seq_emb: from text encoder (may be None)
        """

        # ~~~ Multi-scale downsampling logic ~~~
        outputs = []
        lengths = []
        ...
        # Merge those multi-scale features with the mel features
        # "out" is the final shape (B, oc, T_mel) => oc e.g. 512
        ...
        if self.with_gru_residual:
            res = out
        out_bt, *_ = self.gru(out.transpose(-2, -1))  # (B, T_mel, oc) if batch_first=True
        out = out_bt.transpose(-2, -1)
        if self.with_gru_residual:
            out = (out + res) / math.sqrt(2)
        out, *_ = self.conv_block2(out)

        # ~~~ (New!) Cross-attention + FiLM after the GRU ~~~
        text_metrics = {}
        if (global_emb is not None or seq_emb is not None) and self.film_global is not None:
            ...
            if seq_emb is not None and self.cross_attention is not None:
                ...
                # once training is more advanced
                if (self.text_direct_proj is not None) and (current_epoch > 5):
                    direct_proj = self.text_direct_proj(global_emb)
                ...
                # If there's a learnable text_impact_factor, use it
                if self.text_impact_factor is not None:
                    raw_factor = torch.sigmoid(self.text_impact_factor)
                    # Just do a simple blend factor from 0..0.25
                    blend_factor = 0.25 * raw_factor
...
class ConditionerNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        self.text_direct_proj = None
        self.text_impact_factor = None
        ...
        if text_encoder_config is not None:
            ...
            self.text_impact_factor = torch.nn.Parameter(torch.tensor(0.4))
        ...
        self.encoder = ConditionerEncoder(
            ...
            use_antialiasing=use_antialiasing,
            #
            # pass text-fusion modules into the encoder
            film_global=self.film_global,
            mel_to_attn=self.mel_to_attn,
            cross_attention=self.cross_attention,
            attn_to_mel=self.attn_to_mel,
            text_direct_proj=self.text_direct_proj,
            text_impact_factor=self.text_impact_factor,
        )
    ...
    def forward(self, x, x_wav=None, train=False, text=None, current_epoch=0):
        ...
        Returns: conditions, (optionally y_hat), (optionally h), etc.
        """
        # This dictionary can be used for logging text-related metrics
        text_metrics = {}
        ...
        # Compute mel features from the waveform
        x_mel = self.input_mel(x_wav)  # shape ~ (B, total_channels, T/160)

        # Possibly run a text encoder (only if text is provided)
        global_emb = None
        seq_emb = None
        if self.text_encoder is not None and text is not None:
            ...
            if len(valid_text) == 0:
                ...
                # (global_emb: (B, film_global_dim), seq_emb: (B, num_tokens, cross_attention_dim) or similar)
        ...
        # Optional precoding on x
        if self.precoding:
            x = self.precoding(x)

        # 1) Initial conv to go from input_channels -> n_channels
        x = self.input_conv(x)  # (B, n_channels, T)

        # 2) Encoder → merges with x_mel, runs GRU, then does text fusion *inside the encoder*
