...
class CrossAttentionBlock(torch.nn.Module):
    ...
    def forward(self, x, cond, x_mask=None, cond_mask=None):
        ...
        ### 04 MAY - NEW DEFINITION (MB SAME) + make attention_temperature = 2.0
        scale = 1.0 / self.temperature          # keep the name, but use reciprocal
        q = x * scale
        ### 04 MAY - NEW DEFINITION (MB SAME)
        ...
        ### ADD 04 MAY
        # immediately zero‑out the padded keys *per head*
        if cond_mask is not None:
            attn_weights = attn_weights.masked_fill(
                cond_mask.unsqueeze(1).unsqueeze(2), 0.0)
        ### ADD 04 MAY    
...
class TextConditioner(torch.nn.Module):
    ...
    def __init__(self, text_encoder_config, film_global_dim, cross_attention_dim, total_channels,
        ...
        self.cross_attention = CrossAttentionBlock(cross_attention_dim, num_heads=num_heads, temperature=max(1.0, attention_temperature))  # 04 May: start neutral
    ...
    def forward(self, x_mel, text, q_pad_mask=None): ### 01 MAY: ADD MASK
        ...
        ### 04 MAY ADD
        if self.last_attn_map is None:          # keep only the first (Mel) map
...
class ConditionerNetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        attention_temperature=2.0               # UPD 04 MAY + RECIPROCAL (MB SAME RESULT)
    ):
        super().__init__()
