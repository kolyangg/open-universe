...
class CrossAttentionBlock(torch.nn.Module):
    def __init__(self, hidden_dim, num_heads=4, temperature: float = 0.6): # 1.0 default (no temp)
        ...
        self.temperature = 0.7     # 09 MAY UPD - UPD temp to 0.7 in this version
    ...
    def forward(self, x, cond, x_mask=None, cond_mask=None, attn_bias=None):    
        text_metrics = {}
        ...
        attn_out, attn_weights = self.cross_attn(
            q, k, cond,
            key_padding_mask=cond_mask,        # padding on keys
            attn_mask=attn_bias,               # ← NEW per-query bias
            need_weights=True,
        ...
        # --- track sparsity -------------------------------------------------
        with torch.no_grad():
            row_entropy = -(attn_weights * (attn_weights + 1e-8).log()).sum(-1)
            text_metrics["attention_row_entropy"] = row_entropy.mean().item()
...
class TextConditioner(torch.nn.Module):
    ...
    def __init__(self, text_encoder_config, film_global_dim, cross_attention_dim, total_channels,
        ...
        # ---- NEW:  learnable bias + time-constant that decide how fast
        #            the SIL column “fades out”
        self.sil_bias       = torch.nn.Parameter(torch.tensor(-1.0))  # start at -1 logit
        self.sil_decay_tau  = 40.0        # ≈ frames until bias→e⁻¹ (tweak)
    ...
    def forward(self, x_mel, text, q_pad_mask=None): ### 01 MAY: ADD MASK
        ...
        # ---- (4) build a per-query bias that gradually DIScourages SIL -----
        Q   = x_mel_attn.size(1)                  # #mel frames after DS
        S   = seq_emb.size(1)                     # #text tokens incl. SIL
        decay = torch.exp(
            -torch.arange(Q, device=x_mel_attn.device).float() / self.sil_decay_tau
        )                                         # 1, e^{-1/τ}, …
        attn_bias = x_mel_attn.new_zeros(Q, S)    # (Q, S)
        attn_bias[:, 0] = self.sil_bias * decay   # only SIL column gets bias
        ...
        x_mel_attn, attn_info, attn_map = self.cross_attention(
            x_mel_attn, seq_emb,
            x_mask=q_pad_mask,
            cond_mask=text_key_mask,
            attn_bias=attn_bias               # ← new argument
        )
