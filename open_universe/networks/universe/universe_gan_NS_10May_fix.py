...
class UniverseGAN(Universe):
    ...
    # #### ADD 04 MAY ####
    # # --- Guided‑attention helper --------------------------------------------
    # @staticmethod
    # def _guided_attn_loss(attn, q_len, s_len, sigma=0.2):
    #     """
    #     attn : [B, Q, S]   – averaged heads (already in .last_attn_map)
    #     q_len, s_len : int – #query and #source tokens (same for whole batch here)
    #     sigma : float      – diagonal width (0.1–0.4 usually works)

    #     Returns   scalar GA loss (mean over batch)
    #     """
    #     device = attn.device
    #     # build guidance matrix G_ij = 1 - exp(-((i/Q - j/S)**2)/(2*sigma^2))
    #     i = torch.linspace(0, 1, q_len, device=device).unsqueeze(1)   # [Q,1]
    #     j = torch.linspace(0, 1, s_len, device=device).unsqueeze(0)   # [1,S]
    #     guide = 1.0 - torch.exp(-((i - j) ** 2) / (2 * sigma ** 2))   # [Q,S]

    #     # element‑wise product → encourage weights on diag (low guide==0)
    #     loss = (attn * guide).mean()
    #     return loss
    # # -------------------------------------------------------------------------
    # #### ADD 04 MAY ####
    ...
    #### UPD 05 MAY ####
    # ---------- in UniverseGAN._guided_attn_loss -------------------------
    # @staticmethod
    ...
    #     """
    #     attn   : [B,Q,S]
    #     q_mask : [B,Q]   True = valid frame
    #     s_mask : [B,S]   True = valid token
    #     """
    #     B, Q, S = attn.shape
    #     device  = attn.device
    
    #     # ----- batch‑independent positional grid -------------------------
    #     #   i ∈ [0,1] along mel frames, j ∈ [0,1] along text tokens
    #     i_pos = torch.linspace(0.0, 1.0, Q, device=device).view(1, Q, 1)   # [1,Q,1]
    #     j_pos = torch.linspace(0.0, 1.0, S, device=device).view(1, 1, S)   # [1,1,S]
    #     guide = 1.0 - torch.exp(-((i_pos - j_pos) ** 2) / (2 * sigma ** 2))  # [1,Q,S]
    
    #     # ----- mask out padded rows / columns ---------------------------
    #     pad = (~q_mask).unsqueeze(-1) | (~s_mask).unsqueeze(1)              # [B,Q,S]
    #     valid = ~pad
    
    #     # loss = (attn.masked_fill(pad, 0.0) * guide).sum() / valid.float().sum().clamp_min(1.0)
    
    #     # return loss
        
    #     ### UPD 06 MAY - lenght‑weighted GA LOSS
    #     per_ex = (attn.masked_fill(pad, 0.0) * guide).sum((-1,-2)) / valid.float().sum((-1,-2)).clamp_min(1.)
    #     return per_ex.mean()                        # length‑weighted batch mean
    #     ### UPD 06 MAY - lenght‑weighted GA LOSS
    ...
    def _guided_attn_loss(attn, q_mask, s_mask, sigma=0.15):
        ...
        # # optional per-sample σ widening: sigma * sqrt(S/Q)
        # # broadcast to [B,1,1] so each batch item can differ
        # sigma_eff = (sigma * torch.sqrt(
        #         s_mask.sum(1, keepdim=True).float() /
        #         (q_mask.sum(1, keepdim=True).float() + 1e-5)
        #     ).clamp_min(0.05)).unsqueeze(-1)          #  → [B,1,1]


        # guide = 1.0 - torch.exp(-((q_pos - s_pos / ratio) ** 2) / (2 * sigma_eff ** 2))
    ...
    def training_step(self, batch, batch_idx):
        ...
        #     attn_map = getattr(self.condition_model.text_conditioner, "last_attn_map", None)
        #     if attn_map is not None:
        #         B, Q, S = attn_map.shape
        #         loss_ga = self._guided_attn_loss(attn_map, Q, S, sigma=0.2)
        ...
        if ga_w > 0.0 and use_text:
            ...
            if None not in (attn_map, q_mask, s_mask):
                ...
                # # ------------ (5) coverage loss ------------------------
                # cov = (attn_map.sum(-1) - 1.0).abs()          # [B,Q]
                # cov = cov.masked_select(q_mask).mean()        # only valid frames
                
                # valid_q = q_mask                        # [B,Q] True = real frame
                # cov     = (attn_map.sum(-1) - 1.0).abs()
                # cov     = cov.masked_select(valid_q).mean()   # length‑aware mean
