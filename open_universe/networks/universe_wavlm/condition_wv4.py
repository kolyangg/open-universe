...
# # 20 Apr version - FINAL (works well but lower metrics vs original MelAdapter version)
...
#         output_channels: int,
#         ds_factor: int,
#         # sample_rate: int = 24_000, # keep 24k for now for comparison, but mb need to change to 16k
#         sample_rate: int = 16_000, # not clear if 16k or 24k is needed here
#         model_name: str = "microsoft/wavlm-base",
#         # model_name: str = "microsoft/wavlm-large", # works but seems to need smaller batch size
#         # model_name: str = "microsoft/wavlm-base-plus-sv", # WavLM-Base-Plus for Speaker Verification (works ok)
...
#         print(f'sample rate input: {sample_rate}')
...
class ConditionerNetwork(torch.nn.Module):
    def __init__(
        ...
        # self.input_mel = WavLMAdapter( ### WRONG (INCORRECT SAMPLE RATE INPUT)
        #     total_channels,
        #     total_ds * input_channels,
        #     n_mel_oversample,
        #     use_weight_norm=use_weight_norm,
