...
from .universe_NS_fix import Universe

# from ...losses.wavlm_loss import WavLMLMOSLoss # NEW WaVLM loss 
...
class UniverseGAN(Universe):
    ...
    def init_losses(self, score_model, condition_model, losses, training):
        ...
        # New WavLM loss
        # self.loss_lmos = WavLMLMOSLoss(model_name="microsoft/wavlm-base")
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ----- perceptual LMOS loss (replaces old mel L1) -----------------
        # l_lmos = self.loss_lmos(y_est, target_original)
        # ------------------------------------------------------------------
        ...
        # l_mos_scaled = self.losses_kwargs.weights.mel_l1 / 12 # TEMP - 12 is a parameter to match scale of l_mos to mel_l1
        
        
        # loss_gen = (
        #     l_score * self.losses_kwargs.weights.score
        #     + l_lmos  * l_mos_scaled  # weight for mel_l1 now weight for l_lmos
        #     + loss_aux * self.losses_kwargs.weights.get("aux", 1.0)
        # )
        ...
        # self.log(
        #     "train/wvlm_lmos", l_lmos, batch_size=mix.shape[0], **self.log_kwargs
        # )
        ...
        if torch.isnan(loss_gen):
            print(
                ...
                # f"Found NaN!! Please investigate. {l_score=} {l_lmos=} "
    ...
    def configure_optimizers(self):
        ...
        params_disc = list(self.loss_mrd.parameters()) + list(  ### as before (double MRD loss)
